...
class Questionnaire extends Model {
  ...
  generateMedicalAlerts() {
    ...
    // Check allergies
    if (
      this.stare_generala?.alergii === "DA" &&
      this.stare_generala?.lista_alergii
    ) {
      alerts.push({
        type: "warning",
    ...
    if (this.conditii_femei?.insarcinata === "DA") {
      alerts.push({
        type: "info",
        message: `SARCINĂ - Luna ${
          this.conditii_femei.luna_sarcina || "nespecificată"
        }`,
        priority: "medium",
        category: "pregnancy",
      });
    ...
    // Check medications
    if (
      this.stare_generala?.medicamente_curente === "DA" &&
      this.stare_generala?.lista_medicamente
    ) {
      alerts.push({
        type: "info",
  ...
  static async updateRecord(id, updates) {
    const [count] = await Questionnaire.update(updates, {
  ...
  static async deleteRecord(id) {
    const count = await Questionnaire.destroy({
      where: { questionnaireid: id },
    });
    return count > 0;
  }
